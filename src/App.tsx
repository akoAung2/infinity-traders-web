...
const ecosystemRoutes = [
  { id: 'vt', path: 'M 250 73 C 350 73, 430 145, 512 234' },
  { id: 'kvb', path: 'M 250 300 C 350 300, 415 300, 490 300' },
  { id: 'ultima', path: 'M 250 527 C 350 527, 430 455, 512 366' },
  { id: 'bft', path: 'M 950 175 C 850 175, 770 195, 688 234' },
  { id: 'select', path: 'M 950 425 C 850 425, 770 405, 688 366' },
];
...
const brokerAccess: BrokerAccess[] = [
  ...
  },
  {
    id: 'kvb',
    name: 'KVB',
    logo: '/brokers/kvb.png',
    logoAlt: 'KVB logo',
    tagline: 'Multi-Asset Market Access',
    platforms: ['MT4', 'MT5', 'KVB App'],
    accounts: [
      { name: 'Standard', spread: 'From 0.25 pips', commission: 'See account terms' },
      { name: 'ECN', spread: 'From 0.0 pips', commission: 'See account terms' },
    ],
    ctaUrl: 'https://kvbcaf.com/tsgc1k2xn',
  },
];
  ...
  >
    ...
    <div className="relative z-10 flex h-full flex-col">
      <div className="flex items-start justify-between gap-6">
        <div className={`broker-logo-wrap ${broker.id === 'kvb' ? 'broker-logo-wrap--kvb' : ''}`}>
          <img src={broker.logo} alt={broker.logoAlt} className={`broker-logo ${broker.id === 'kvb' ? 'broker-logo--kvb' : ''}`} />
        </div>
...
const InfinityHeroAtmosphere = () => {
  ...
  const springX = useSpring(pointerX, { stiffness: 75, damping: 24, mass: 0.9 });
  const springY = useSpring(pointerY, { stiffness: 75, damping: 24, mass: 0.9 });
  const redX = useTransform(springX, [0, 100], [18, 82]);
  const redY = useTransform(springY, [0, 100], [24, 70]);
  const orangeX = useTransform(springX, [0, 100], [80, 20]);
  const orangeY = useTransform(springY, [0, 100], [72, 16]);
  const emberX = useTransform(springX, [0, 100], [35, 65]);
  const emberY = useTransform(springY, [0, 100], [78, 22]);
  const coolX = useTransform(springX, [0, 100], [12, 88]);
  const coolY = useTransform(springY, [0, 100], [76, 26]);
  ...
  return (
    <div ref={atmosphereRef} className="hero-mouse-atmosphere" aria-hidden="true">
      <div className="hero-bubble-noise" />
      <div className="hero-mouse-cursor-glow" />
      <motion.div className="hero-mouse-orb hero-mouse-orb-red" style={{ left: redX, top: redY }} animate={reduceMotion ? undefined : { scale: [0.96, 1.05, 0.98], opacity: [0.24, 0.34, 0.24] }} transition={{ duration: 12, ease: 'easeInOut', repeat: Infinity }} />
      <motion.div className="hero-mouse-orb hero-mouse-orb-orange" style={{ left: orangeX, top: orangeY }} animate={reduceMotion ? undefined : { scale: [1.02, 0.94, 1.04], opacity: [0.12, 0.22, 0.12] }} transition={{ duration: 15, ease: 'easeInOut', repeat: Infinity }} />
      <motion.div className="hero-mouse-orb hero-mouse-orb-ember" style={{ left: emberX, top: emberY }} animate={reduceMotion ? undefined : { scale: [0.92, 1.08, 0.92], opacity: [0.08, 0.18, 0.08] }} transition={{ duration: 18, ease: 'easeInOut', repeat: Infinity }} />
      <motion.div className="hero-mouse-orb hero-mouse-orb-cool" style={{ left: coolX, top: coolY }} animate={reduceMotion ? undefined : { scale: [1.04, 0.96, 1.04], opacity: [0.06, 0.13, 0.06] }} transition={{ duration: 20, ease: 'easeInOut', repeat: Infinity }} />
            ...
            <div className="hero-marquee-track flex w-max items-center">
              {[0, 1].map((copy) => (
                <div key={copy} className="flex shrink-0 items-center gap-6 pr-6 md:gap-10 md:pr-10">
                  {['VT Markets', 'Ultima Markets', 'KVB', 'Trading Ecosystem', 'Infinity Trader'].map((name) => (
      ...
      <section className="ecosystem-section relative min-h-[100svh] overflow-hidden py-16 md:py-20">
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-6 md:px-10">
          ...
          <div className="ecosystem-map mt-12 md:mt-14">
            <svg className="ecosystem-routes" viewBox="0 0 1200 600" aria-hidden="true" preserveAspectRatio="none">
              {ecosystemRoutes.map((route) => <path key={route.id} id={`route-${route.id}`} d={route.path} />)}
              {!reduceMotion && ecosystemRoutes.map((route, index) => <circle key={`${route.id}-pulse`} className="route-pulse" r="4"><animateMotion dur="5.4s" begin={`${index * 0.85}s`} repeatCount="indefinite"><mpath href={`#route-${route.id}`} /></animateMotion></circle>)}
            </svg>
            ...
            {[
              ...
              { name: 'KVB', category: 'BROKER PARTNER', logo: '/brokers/kvb.png', alt: 'KVB', position: 'node-kvb', id: 'kvb' },
              ...
              <motion.div key={partner.name} initial={reduceMotion ? false : { opacity: 0, y: 16 }} whileInView={reduceMotion ? undefined : { opacity: 1, y: 0 }} whileHover={reduceMotion ? undefined : { y: -3 }} viewport={revealViewport} transition={{ delay: 0.08 * index, duration: 0.6, ease: [0.16, 1, 0.3, 1] }} className={`ecosystem-node ${partner.position}`}>
                <div className="ecosystem-node-top"><span><i />COLLABORATION</span></div>
                <div className="ecosystem-logo-frame"><img src={partner.logo} alt={`${partner.alt} logo`} className={partner.id === 'kvb' ? 'ecosystem-logo--kvb' : partner.id === 'select' ? 'ecosystem-logo--select' : undefined} /></div>
          ...
          <div className="ecosystem-marquee"><div className="ecosystem-marquee-track">{[0, 1].map((copy) => <div key={copy} className="ecosystem-marquee-copy">{['VT Markets', 'Ultima Markets', 'KVB', 'Burmese Funded Trader', 'Select2Notion'].map((name) => <span key={`${copy}-${name}`}>{name}<b>•</b></span>)}</div>)}</div></div>
